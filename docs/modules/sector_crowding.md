...
2. **成交量**：板块集中度 = 板块成交量 ÷ 全A成交量 × 100%，行业板块绘制占比折线图。
3. **成交额**：板块集中度 = 板块成交额 ÷ 全A成交额 × 100%，行业板块绘制占比折线图。

行业板块区域以**板块成交占比**（板块集中度）为主指标并绘制三年折线图；
下方指数卡片仍显示前 5% 个股成交集中度（高于 45% 标记拥挤）。
成交量 / 成交额维度另附**涨跌幅榜成交额占比**卡片：每日涨幅榜 / 跌幅榜
前 5% 个股成交额占全天成交额比重，卡片内左右并排两张折线图
（左：涨幅榜，右：跌幅榜）。
...
### 板块集中度（板块成交占比，行业板块区域主指标）

$$
\text{板块成交占比} = \frac{\text{板块当日成交量（成交额）}}{\text{全A当日成交量（成交额）}} \times 100\%
$$

- 全A 口径：当日全部已上市 A 股中具有行业分类（`stock_basic.industry`）个股的
  成交量（成交额）合计（与行业维度同一股票宇宙，因此各板块占比之和恒为 100%）。
- 行业历史新增字段：`vol_market_share_pct`（板块成交量 ÷ 全A成交量 × 100%）、
  `amount_market_share_pct`（板块成交额 ÷ 全A成交额 × 100%）。
- 行业板块区域以「板块成交占比」替换前 5% 个股成交集中度：排行表主列为
  板块成交占比（按三年分位着色），点击行业行绘制该板块占比的折线图；
  顶部统计含占比最高板块、占比 TOP5 合计、全A成交额 / 全A成交量。
- 数据层在 `fetch_day` 拉取时自动计算；历史数据回填（本地计算，不请求接口）：
...
$$
\text{涨跌幅榜占比} = \frac{\text{涨幅榜 / 跌幅榜前 5% 个股成交额合计}}{\text{全A当日成交额}} \times 100\%
$$

- 涨幅榜：当日全部 A 股（具有行业分类、成交额 > 0）按 `pct_chg` 降序取
  前 k = max(1, ceil(股票数 × 5%)) 只；跌幅榜按 `pct_chg` 升序取前 k 只。
- 该指标需要逐日 `pro.daily` 的 `pct_chg`，历史缓存未存个股涨跌幅，
  因此历史回填会对每个交易日补拉一次日行情（与全A回填同一模式）：

```bash
python scripts/backfill_trading_crowding_extreme.py
```

- 缓存：`data/trading_crowding/trading_crowding_extreme_history.csv`，
  面板以全宽卡片左右并排展示涨幅榜 / 跌幅榜两张占比折线图，
